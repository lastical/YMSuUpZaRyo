...
st.sidebar.image("05source/OASIS.png", caption="OASIS")
...
#칼럼
    ...
    with col1:
        st.subheader("Vocal")
        st.image("05source/Liam.png", caption="Liam Gallagher", use_container_width=True)
        st.write("Frontman, vocalist")

    with col2:
        st.subheader("Guitar")
        st.image("05source/Noel.png", caption="Noel Gallagher", use_container_width=True)
...
with tab1:
    st.video("05source/WonderWall.mp4")
...
with tab2:
    st.video("05source/Dont Look Back In Anger.mp4")
...
with st.expander("Live at Knebworth"):
    st.image("05source/Knebworth.png", caption="Live at knebworth")
...
# 페이지 이동 처리
if option == "창설":
    st.image("05source/CHANG.png", caption="홈 페이지 이미지")
    st.write("창설 직후 사진입니다. 이때는 리암과 노엘이 함께 했습니다.")
elif option == "분열":
    st.image("05source/BEADY.png", caption="분열 후 이미지")
    st.write("분열 후 비디아이로 활동하는 리암의 모습입니다")
elif option == "재결합":
    st.image("05source/OASISJAE.png", caption="재결합 후 이미지")
...
st.video("05source/Whatever.mp4")
